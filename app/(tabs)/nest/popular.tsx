...
import { CommunityCollectionSummary, fetchCommunityPopularApi } from "@/services/api/community";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetchCommunityPopularApi();
        if (mounted) setItems(res.items ?? []);
      } catch (e) {
        console.log("[NestPopularScreen] ERROR", e);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <View style={styles.headerWrap}>
        <SimpleHeader title="요즘 인기 있는 새록" circleBackButton />
      </View>
                ...
                <View key={item.collectionId}>
                  <CommunityCollectionRow
                    ...
                    onPress={() => {
                      ...
                      openSaerokDetail(router, item.collectionId, { from: "nest_popular" });
                    }}
                  />
                  {idx < items.length - 1 ? (
