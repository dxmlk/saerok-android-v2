...
import { CommunityCollectionSummary, fetchCommunityRecentApi } from "@/services/api/community";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetchCommunityRecentApi();
        if (mounted) setItems(res.items ?? []);
      } catch (e) {
        console.log("[NestRecentScreen] ERROR", e);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <View style={styles.headerWrap}>
        <SimpleHeader title="최근에 올라온 새록" circleBackButton />
      </View>
                ...
                <View key={item.collectionId}>
                  <CommunityCollectionRow
                    ...
                    onPress={() => {
                      ...
                      openSaerokDetail(router, item.collectionId, { from: "nest_recent" });
                    }}
                  />
                  {idx < items.length - 1 ? (
